...
const FormScreen = () => {
  ...
  const submitHandler = async (e) => {
    ...
    if (response.ok) {
        // שליחת הבקשה השנייה עם mode: 'no-cors'
        ...
        await fetch(secondApiUrl, {
            mode: 'no-cors'  // הוספת mode: 'no-cors' לפתרון בעיית ה-CORS
        });

        // ניקוי הטופס והצגת ההודעה
      ...
      <form className={styles.form}>
        <div className={styles.column}>
          ...
          <label className={styles.checkboxContainer}>
            ...
            מאשרת קבלת דיוור במייל ואת התקנון ומדיניות הפרטיות
          </label>
